...
The M4 application/UI baseline includes the `SourceDraft` load/validate/save workflow, the controlled supported-field catalog with generic Mention/Claim editors, versioned Source Type Template configuration and the configurable Source Acquisition workspace. `SourceEditor`/`SourceWorkspacePage` keeps Source metadata and asset management together with two independently switchable workspace panels for Source asset/scan preview, transcription, translation, and Mentions & Claims. Desktop panels are resizable and scroll independently; narrow layouts stack vertically. All panel edits still compose into one Source-scoped edit and persist through the Application `SourceDraft` workflow. Source asset previews are read through the Application `ReadSourceAsset`/`SourceAssetStorage` boundary rather than exposing storage paths directly to Filament.
