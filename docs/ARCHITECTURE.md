...
The M4 application/UI baseline includes the `SourceDraft` load/validate/save workflow, the controlled supported-field catalog with generic Mention/Claim editors, versioned Source Type Template configuration and the configurable Source Acquisition workspace. `SourceEditor`/`SourceWorkspacePage` keeps Source metadata and asset management together with two independently switchable workspace panels for Source asset/scan preview, transcription, translation, Mentions & Claims, and a read-only YAML evidence graph. Desktop panels are resizable and scroll independently; narrow layouts stack vertically. All editable panel state still composes into one Source-scoped edit and persists through the Application `SourceDraft` workflow. The YAML graph is a deterministic Application projection of the same current/proposed Mention/Claim state and can be downloaded without introducing another persistence model. The persisted form of the same projection is also loaded lazily into the read-only Source details drawer from the Sources list, avoiding graph loading for every browse-row render. Source asset previews are read through the Application `ReadSourceAsset`/`SourceAssetStorage` boundary rather than exposing storage paths directly to Filament.
