...
/**
 * Integrated Basic Source Acquisition workspace.
 *
 * Source Type Templates are presentation configuration only. All semantic writes
 * are expressed as one SourceDraftChanges operation and committed through the
 * existing SaveSourceDraft boundary.
 *
 * @property-read Schema $form
 */
abstract class BasicSourceEditorPage extends Page
{
    ...
    public function getSubheading(): string
    {
        if ($this->sourceId === null) {
            return sprintf('%s · Basic acquisition workspace · unsaved', $this->sourceTypeContext);
        }

        return sprintf(
            'Source %s · %s · revision %d · Basic acquisition workspace',
            $this->sourceId,
    ...
    public function sourceTypeChanged(): void
    {
        ...
        if ($this->findSelectableTemplate($selectedTemplateId) === null) {
    ...
    public function templateChanged(?string $templateId): void
    {
        ...
        $state = is_array($this->data) ? $this->data : [];
        $state = app(StructuredAcquisitionFormAdapter::class)->applyTemplatePresentation(
            $state,
    ...
    public function save(): void
    {
        ...
        try {
            ...
            $structuredInput = app(StructuredAcquisitionFormAdapter::class)->editInput($data);
        ...
        $notification = Notification::make()
            ...
            ->body($result->changed
                ? 'Source, Mention/Claim revisions, and the resulting EvidenceState were updated atomically.'
